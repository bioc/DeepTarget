...
#
#saveRDS(Pwy.Enr,file = 'Result/Pwy.Enrichment.RDS')
```
...
DrugTargetSim <- PredTarget(List.sim,metadata)
...
for (i in 1:nrow(sec.prism.f)){
    ...
    row.names(DRS) <- row.names(sec.prism.f)[i]
    ## for mutant
    Out.M <- DoInteractMutant(DrugTargetSim[i,],d.mt,DRS,KO.GES)
    ...
    ## for expression.
    Out.Expr <- DoInteractExp(DrugTargetSim[i,],d.expr,DRS,KO.GES,CutOff= 2)
